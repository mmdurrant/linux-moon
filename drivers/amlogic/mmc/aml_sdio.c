...
/* pr_info("SDIO_CONF=0x%x\n",readl(host->base+SDIO_CONF)); */
/* pr_info("CBUS_SDIO_CONF=0x%x\n",aml_read_cbus(CBUS_SDIO_CONF)); */
...
static void aml_sdio_print_err(struct amlsd_host *host, char *msg)
{
	...
	u32 clk_rate = host->clksrc_rate;/* clk_get_rate(clk_src)/2; */
...
/* mmc->ops->enable_sdio_irq(mmc, 0); */
	...
	if (!mrq->data)
		...
		timeout = 5000;/* mod_timer(&host->timeout_tlist, */
		/* 5s */
/* if(mmc->caps & MMC_CAP_SDIO_IRQ){ */
...
irqreturn_t aml_sdio_irq_thread(int irq, void *data)
{
	...
	/* pr_info(KERN_DEBUG "TIME_STAMP: %8d[%s]\n",
...
static void aml_sdio_set_clk_rate(struct amlsd_platform *pdata, u32 clk_ios)
{
	...
	u32 clk_rate = host->clksrc_rate/2; /* tmp for 3.10 */
	u32 clk_div;
	...
	/* sdio_err("Clk IOS %d, Clk Src %d, Host Max Clk %d,
	 *clk_divide=%d, actual_clock=%d\n", */
		/* clk_ios, (clk_rate*2), pdata->f_max,
		 * clk_div, pdata->mmc->actual_clock); */
}
...
{
	/* struct amlsd_host *host =
	container_of(class, struct amlsd_host, debug); */

	count = sscanf(buf, "%x", &sdio_debug_flag);
...
static struct amlsd_host *aml_sdio_init_host(struct amlsd_host *host)
{
	...
	if (request_threaded_irq(host->irq, (irq_handler_t)aml_sdio_irq,
...
/*	if (amlsd_get_reg_base(pdev, host))
	...
	for (i = 0; i < MMC_MAX_DEVICE; i++) {
		...
		it will conflict with aml_sdio_request */
		/* host->mmc = mmc; */
		pdata->mmc = mmc;
